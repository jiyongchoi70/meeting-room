...
-- lookup 180 (앱 코드와 동일): 110=신청, 120=승인, 130=반려, 140=완료(자동 부여만; 본 RPC는 승인/반려만)
...
BEGIN
  ...
  IF p_next_status NOT IN (120, 130) THEN
    RAISE EXCEPTION '[E_INVALID_STATE] next_status는 120(승인), 130(반려)만 허용됩니다.';
  END IF;
  ...
  IF v_all_valid IS DISTINCT FROM true THEN
    RAISE EXCEPTION '[E_INVALID_STATE] 상태 전이가 허용되지 않습니다. 신청(110)→승인(120)/반려(130)만 가능합니다.';
  END IF;
...
COMMENT ON FUNCTION public.rpc_change_reservation_status(text, uuid, integer, text, text) IS
  '승인(120)/반려(130)만. 단일 앵커 + this|all 시리즈. 담당자(user_type 110) 또는 mr_approver 권한 검증.';
...
COMMENT ON FUNCTION public.rpc_change_reservation_status_many(text, uuid[], integer, text) IS
  '승인(120)/반려(130)만. 선택 ID 각각 단건(시리즈 자동 확장 없음). 그리드 일괄 승인/반려 등.';
